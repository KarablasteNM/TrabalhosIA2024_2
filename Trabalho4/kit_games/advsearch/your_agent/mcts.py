...
def monte_carlo_movimento(state, iteracoes=10):
            ...
            for _ in range(iteracoes):
                ...
                while not state_copy.is_terminal():
                    ...
                    if movimentos_validos_internos:
                        state_copy = state_copy.next_state(melhor_movimento_atual(movimentos_validos_internos))
...
def melhor_movimento_atual(movimentos_validos):
    maior_valor = -51   
    for i in movimentos_validos:
        x,y = i
        if EVAL_TEMPLATE[x][y] > maior_valor:
            melhor_movimento = i
            maior_valor = EVAL_TEMPLATE[x][y]
    return melhor_movimento